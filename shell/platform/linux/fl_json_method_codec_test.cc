...
// Convert a binary blob to a string
...
// Convert a string to a binary blob
static GBytes* text_to_message(const gchar* text) {
  return g_bytes_new(text, strlen(text));
}

// Encode a method call using JsonMethodCodec to a UTF-8 string.
...
// Encode a success envelope response using JsonMethodCodec to a UTF-8 string.
...
// Encode a error envelope response using JsonMethodCodec to a UTF8 string.
...
// Decode a method call using JsonMethodCodec with a UTF8 string.
...
// Decode a method call using JsonMethodCodec. Expect the given error.
...
// Decode a response using JsonMethodCodec. Expect the response is a result.
...
// Decode a response using JsonMethodCodec. Expect the response contains the
// given error.
...
TEST(FlJsonMethodCodecTest, DecodeMethodCallInvalidJson) {
  decode_error_method_call("X", FL_MESSAGE_CODEC_ERROR,
                           FL_MESSAGE_CODEC_ERROR_FAILED);
}
...
TEST(FlJsonMethodCodecTest, DecodeMethodCallExtraData) {
  decode_error_method_call("{\"method\":\"hello\"}XXX", FL_MESSAGE_CODEC_ERROR,
...
TEST(FlJsonMethodCodecTest, DecodeResponseNoTerminator) {
  decode_error_response("[42", FL_MESSAGE_CODEC_ERROR,
                        FL_MESSAGE_CODEC_ERROR_OUT_OF_DATA);
}

TEST(FlJsonMethodCodecTest, DecodeResponseInvalidJson) {
  decode_error_response("X", FL_MESSAGE_CODEC_ERROR,
                        FL_MESSAGE_CODEC_ERROR_FAILED);
}
...
TEST(FlJsonMethodCodecTest, DecodeResponseSuccessExtraData) {
  decode_error_response("[null]X", FL_MESSAGE_CODEC_ERROR,
                        FL_MESSAGE_CODEC_ERROR_ADDITIONAL_DATA);
}

TEST(FlJsonMethodCodecTest, DecodeResponseErrorExtraData) {
  decode_error_response("[\"error\",null,null]X", FL_MESSAGE_CODEC_ERROR,
                        FL_MESSAGE_CODEC_ERROR_ADDITIONAL_DATA);
}
