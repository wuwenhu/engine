...
G_MODULE_EXPORT FlValue* fl_method_response_get_result(FlMethodResponse* self,
  ...
  if (FL_IS_METHOD_ERROR_RESPONSE(self)) {
    ...
    // FIXME(robert-ancell) Encode the error_details with JSON and show in the
    // message FlValue *details =
    // fl_method_error_response_get_details(FL_METHOD_ERROR_RESPONSE(self));
    g_autofree gchar* text = nullptr;
    if (message != nullptr)
      text =
          g_strdup_printf("Remote code returned error %s: %s", code, message);
    else
      text = g_strdup_printf("Remote code returned error %s", code);
    g_set_error_literal(error, FL_METHOD_RESPONSE_ERROR,
                        FL_METHOD_RESPONSE_ERROR_REMOTE_ERROR, text);
    return nullptr;
...
G_MODULE_EXPORT FlMethodSuccessResponse* fl_method_success_response_new(
    FlValue* result) {
  FlMethodSuccessResponse* self = static_cast<FlMethodSuccessResponse*>(
      g_object_new(fl_method_success_response_get_type(), nullptr));
  g_return_val_if_fail(result != nullptr, nullptr);
...
G_MODULE_EXPORT FlMethodErrorResponse* fl_method_error_response_new(
  ...
  FlMethodErrorResponse* self = static_cast<FlMethodErrorResponse*>(
...
fl_method_not_implemented_response_new() {
  return static_cast<FlMethodNotImplementedResponse*>(
